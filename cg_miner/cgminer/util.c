...
void *_cgmalloc(size_t size, const char *file, const char *func, const int line)
{
	...
	ret = malloc(size);
	if (unlikely(!ret))
		applog(LOG_ERR, "Failed to malloc size %d from %s %s:%d", (int)size, file, func, line);
	return ret;
}
...
void *_cgcalloc(const size_t memb, size_t size, const char *file, const char *func, const int line)
{
	...
	if (unlikely(!ret))
		applog(LOG_ERR, "Failed to calloc memb %d size %d from %s %s:%d", (int)memb, (int)size, file, func, line);
	return ret;
}
...
void *_cgrealloc(void *ptr, size_t size, const char *file, const char *func, const int line)
{
	...
	if (unlikely(!ret))
		applog(LOG_ERR, "Failed to realloc size %d from %s %s:%d", (int)size, file, func, line);
...
json_t *json_web_config(const char *url)
{
	...
	curl = curl_easy_init();
	if (unlikely(!curl))
		quithere(ERR_POOL_EXIT, "CURL initialisation failed");
...
void check_extranonce_option(struct pool *pool, char * url)
{
	...
	{
		strncpy(extra_op, extra_op_loc, sizeof(extra_op) - 1);
...
retry:
		...
		{
			sent = mbedtls_ssl_write(&pool->mtls_data.ssl, (unsigned char *)(s + ssent), len);
		}
#endif
		if (sent < 0) {
			if (!sock_blocks())
	...
	do {
		if (pool->sock)
		{
			if(!pool->is_tls)
				...
				n = mbedtls_ssl_read( &pool->mtls_data.ssl, (unsigned char *)pool->sockbuf, RECVSIZE);
	...
	if (!strstr(pool->sockbuf, "\n")) {
		...
		do {
			...
			if(!pool->is_tls)
			{
				...
				if (!n) {
					applog(LOG_ERR, "Socket closed waiting in recv_line");
					suspend_stratum(pool);
					break;
				}
			}
			...
			{
				 n = mbedtls_ssl_read( &pool->mtls_data.ssl, (unsigned char *)s, RECVSIZE);
				if (!n) {
					applog(LOG_ERR, "Socket closed waiting in recv_line");
					suspend_stratum(pool);
					break;
			...
			if (n < 0) {
				...
				{
					applog(LOG_DEBUG, "Failed to recv sock(mem not enough) in recv_line");
					suspend_stratum(pool);
					break;
				}
			}
...
static void get_vmask(struct pool *pool, char *bbversion)
{
	...
	for (i = 0; i < 16; i++) {
		...
		applog(LOG_DEBUG, "Pool %d version_mask:0x%08x", pool->pool_no, uiMagicNum);

		pool->vmask_001[i] = bswap_32(uiMagicNum);
	}
	...
	for (i = 0; i < 16; i++) {
			...
			memcpy(pool->vmask_002[i], tmpstr, 9);
			applog(LOG_DEBUG, "tmpstr:%s", tmpstr);
...
static bool set_vmask(struct pool *pool, json_t *val)
{
	...
	pool->vmask_003[cnt++] = 0;
	pool->vmask_003[cnt++] = mask;

	for (i = 15; i <= 28; i++) {
		rem = (mask >> i) & 0x1;
		if (rem)
			pool->vmask_003[cnt++] = 1 << i;
	}

	if (cnt < 8)
		applog(LOG_ERR, "Pool %d version mask count:%d and < 8", pool->pool_no, cnt);
...
static bool configure_stratum_mining(struct pool *pool)
{
	...
	if (!response_str)
	{
		applog(LOG_ERR, "recv_line is error %s", __FUNCTION__);
		goto out;
	}
...
static bool parse_notify(struct pool *pool, json_t *val)
{
	...
	free(pool->coinbase);
	pool->coinbase = cgcalloc(1, alloc_len);
...
static bool parse_extranonce(struct pool *pool, json_t *val)
{
        ...
        free(pool->nonce1bin);
		pool->nonce1bin = cgcalloc(1, pool->n1_len);
        if (unlikely(!pool->nonce1bin))
                quithere(ERR_POOL_EXIT, "Failed to calloc pool->nonce1bin");
...
static void __suspend_stratum(struct pool *pool)
{
	...
	if(pool->is_tls)
	{
		mbedtls_ssl_close_notify( &pool->mtls_data.ssl);
		mbedtls_net_free(&pool->mtls_data.sockfd);
		mbedtls_ssl_free(&pool->mtls_data.ssl);
		mbedtls_ssl_config_free(&pool->mtls_data.conf);
		mbedtls_ctr_drbg_free(&pool->mtls_data.ctr_drbg);
		mbedtls_entropy_free(&pool->mtls_data.entropy);
...
retry:
	...
	if (!pool->sockbuf) {
		pool->sockbuf = cgcalloc(1, RBUFSIZE);
		if (likely(pool->sockbuf))
	...
	if(pool->is_tls)
	{
		mbedtls_ctr_drbg_init(&pool->mtls_data.ctr_drbg);
		mbedtls_net_init(&pool->mtls_data.sockfd);
		mbedtls_ssl_init(&pool->mtls_data.ssl);
		mbedtls_ssl_config_init(&pool->mtls_data.conf);
		mbedtls_entropy_init( &pool->mtls_data.entropy );
		const char *pers = "mini_client";
	    if( mbedtls_ctr_drbg_seed( &pool->mtls_data.ctr_drbg, mbedtls_entropy_func, &pool->mtls_data.entropy,
                       (const unsigned char *) pers, strlen( pers ) ) != 0 )
		{
			return -1;
		}
		if( mbedtls_ssl_config_defaults( &pool->mtls_data.conf,
    	...
    	mbedtls_ssl_conf_authmode( &pool->mtls_data.conf, MBEDTLS_SSL_VERIFY_NONE );
    	mbedtls_ssl_conf_rng( &pool->mtls_data.conf, mbedtls_ctr_drbg_random, &pool->mtls_data.ctr_drbg );
    	if( mbedtls_ssl_setup( &pool->mtls_data.ssl, &pool->mtls_data.conf ) != 0 )
    	{
			return -1;
    	}
		pool->mtls_data.sockfd.fd = sockd;
		mbedtls_ssl_set_bio( &pool->mtls_data.ssl, &pool->mtls_data.sockfd, mbedtls_net_send, mbedtls_net_recv, NULL );
		if(mbedtls_ssl_handshake(&pool->mtls_data.ssl) != 0)
		{
			return -1;
		}
	}
...
rereceive:
	...
	if (unlikely(!pool->nonce1bin))
	{
		pool->n1_len = 0;
		cg_wunlock(&pool->data_lock);
		applog(LOG_INFO, "Failed to calloc nonce1bin in initiate_stratum");
		free(sessionid);
		free(nonce1);
		goto out;
	}
...
void *str_text(char *ptr)
{
	...
	if (ptr == NULL) {
		...
		if (unlikely(!ret))
			quithere(ERR_CREATE_EXIT, "Failed to malloc null");
	}
...
void _cgsem_init(cgsem_t *cgsem, const char *file, const char *func, const int line)
{
	...
	if (pipe(cgsem->pipefd) == -1)
		quitfrom(ERR_CREATE_EXIT, file, func, line, "Failed pipe errno=%d", errno);
	...
	for (i = 0; i < 2; i++) {
		...
		if (fcntl(fd, F_SETFD, flags) == -1)
			quitfrom(ERR_CREATE_EXIT, file, func, line, "Failed to fcntl errno=%d", errno);
	}
}
...
retry:
	...
	if (interrupted())
		goto retry;
	quitfrom(ERR_CREATE_EXIT, file, func, line, "Failed to sem_timedwait errno=%d cgsem=0x%p", errno, cgsem);
	/* We don't reach here */
	return 0;
}
...
void _cgsem_init(cgsem_t *cgsem, const char *file, const char *func, const int line)
{
	...
	if ((ret = sem_init(cgsem, 0, 0)))
		quitfrom(ERR_CREATE_EXIT, file, func, line, "Failed to sem_init ret=%d errno=%d", ret, errno);
}

void _cgsem_post(cgsem_t *cgsem, const char *file, const char *func, const int line)
{
	if (unlikely(sem_post(cgsem)))
		quitfrom(ERR_CREATE_EXIT, file, func, line, "Failed to sem_post errno=%d cgsem=0x%p", errno, cgsem);
}
...
retry:
	if (unlikely(sem_wait(cgsem))) {
		if (interrupted())
			goto retry;
		quitfrom(ERR_CREATE_EXIT, file, func, line, "Failed to sem_wait errno=%d cgsem=0x%p", errno, cgsem);
	}
}
...
void str_to_hex(char *s, int l, char *d)
{
    while(l--)
    {
...
void hex_to_str(char *s, int l, char *d)
{
    while(l--)
    {
        char* p1 = s+l;
