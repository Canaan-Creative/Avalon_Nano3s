...
#define ABORT_EXIT				9
#define ERR_SIGNAL				9
#define ERR_OPT_EXIT			2
#define ERR_CREATE_EXIT			3
#define ERR_POOL_EXIT			4
...
static inline void _mutex_lock(pthread_mutex_t *lock, const char *file, const char *func, const int line)
{
	...
	if (unlikely(ret = pthread_mutex_lock(lock)))
		quitfrom(ERR_CREATE_EXIT, file, func, line, "WTF MUTEX ERROR ON LOCK! errno=%d", ret);
...
{
	int ret = 0;
	if (unlikely(ret = pthread_mutex_unlock(lock)))
		quitfrom(ERR_CREATE_EXIT, file, func, line, "WTF MUTEX ERROR ON UNLOCK! errno=%d", ret);
...
static inline void _wr_lock(pthread_rwlock_t *lock, const char *file, const char *func, const int line)
{
	...
	if (unlikely(pthread_rwlock_wrlock(lock)))
		quitfrom(ERR_CREATE_EXIT, file, func, line, "WTF WRLOCK ERROR ON LOCK! errno=%d", errno);
	GOTLOCK(lock, file, func, line);
}
...
static inline void _rd_lock(pthread_rwlock_t *lock, const char *file, const char *func, const int line)
{
	...
	if (unlikely(pthread_rwlock_rdlock(lock)))
		quitfrom(ERR_CREATE_EXIT, file, func, line, "WTF RDLOCK ERROR ON LOCK! errno=%d", errno);
	GOTLOCK(lock, file, func, line);
}
...
static inline void _rw_unlock(pthread_rwlock_t *lock, const char *file, const char *func, const int line)
{
	if (unlikely(pthread_rwlock_unlock(lock)))
		quitfrom(ERR_CREATE_EXIT, file, func, line, "WTF RWLOCK ERROR ON UNLOCK! errno=%d", errno);
	GUNLOCK(lock, file, func, line);
}
...
static inline void _mutex_init(pthread_mutex_t *lock, const char *file, const char *func, const int line)
{
	if (unlikely(pthread_mutex_init(lock, NULL)))
		quitfrom(ERR_CREATE_EXIT, file, func, line, "Failed to pthread_mutex_init errno=%d", errno);
...
static inline void _rwlock_init(pthread_rwlock_t *lock, const char *file, const char *func, const int line)
{
	if (unlikely(pthread_rwlock_init(lock, NULL)))
		quitfrom(ERR_CREATE_EXIT, file, func, line, "Failed to pthread_rwlock_init errno=%d", errno);
...
#include <stdatomic.h>
extern atomic_bool flag_api_ready;
...
struct pool {
	...
	struct timeval sshare_send; //share send start time
};
...
struct work {
	...
	unsigned char   midstate4[32];
	unsigned char   midstate5[32];
	unsigned char   midstate6[32];
	unsigned char   midstate7[32];
...
struct device_drv {
	...
	void (*get_login)(struct cgpu_info *,char *user,char *pass);
	void (*get_set_web_stats)(struct cgpu_info *,void * ,uint8_t );
...
#define tailsprintf(buf, bufsiz, fmt, ...) do { \
	...
	{ \
		applog(LOG_ERR, "tailsprintf buffer overflow in %s %s line %d", __FILE__, __func__, __LINE__); \
		strncat(buf, tmp13, bufsiz - buflen - 1); \
	} \
	else \
		strcat(buf, tmp13); \
} while (0)
...
extern void clear_new_nonce(struct thr_info *thr);
extern int submit_nonce(struct thr_info *thr, struct work *work, uint32_t nonce);
