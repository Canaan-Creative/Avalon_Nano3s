...
static void k_alloc_items(K_LIST *list, KLIST_FFL_ARGS)
{
	...
	if (list->is_store) {
		quithere(ERR_CREATE_EXIT, "List %s store can't %s()" KLIST_FFL,
	...
	if (!(list->item_memory = realloc(list->item_memory,
		...
		quithere(ERR_CREATE_EXIT, "List %s item_memory failed to realloc count=%d",
				list->name, list->item_mem_count);
	}
	item = calloc(allocate, sizeof(*item));
	if (!item) {
		quithere(ERR_CREATE_EXIT, "List %s failed to calloc %d new items - total was %d, limit was %d",
	...
	while (item) {
		...
		if (!(list->data_memory = realloc(list->data_memory,
			...
			quithere(ERR_CREATE_EXIT, "List %s data_memory failed to realloc count=%d",
					list->name, list->data_mem_count);
		}
		item->data = calloc(1, list->siz);
		if (!(item->data))
			quithere(ERR_CREATE_EXIT, "List %s failed to calloc item data", list->name);
...
K_STORE *k_new_store(K_LIST *list)
{
	...
	if (!store)
		quithere(ERR_CREATE_EXIT, "Failed to calloc store for %s", list->name);
...
K_LIST *_k_new_list(const char *name, size_t siz, int allocate, int limit, bool do_tail, KLIST_FFL_ARGS)
{
	...
	if (allocate < 1)
		quithere(ERR_CREATE_EXIT, "Invalid new list %s with allocate %d must be > 0", name, allocate);

	if (limit < 0)
		quithere(ERR_CREATE_EXIT, "Invalid new list %s with limit %d must be >= 0", name, limit);

	list = calloc(1, sizeof(*list));
	if (!list)
		quithere(ERR_CREATE_EXIT, "Failed to calloc list %s", name);

	list->is_store = false;

	list->lock = calloc(1, sizeof(*(list->lock)));
	if (!(list->lock))
		quithere(ERR_CREATE_EXIT, "Failed to calloc lock for list %s", name);
...
K_ITEM *_k_unlink_tail(K_LIST *list, KLIST_FFL_ARGS)
{
	...
	if (!(list->do_tail)) {
		quithere(ERR_CREATE_EXIT, "List %s can't %s() - do_tail is false" KLIST_FFL,
				list->name, __func__, KLIST_FFL_PASS);
	}
...
void _k_add_head(K_LIST *list, K_ITEM *item, KLIST_FFL_ARGS)
{
	if (item->name != list->name) {
		quithere(ERR_CREATE_EXIT, "List %s can't %s() a %s item" KLIST_FFL,
...
void _k_insert_before(K_LIST *list, K_ITEM *item, K_ITEM *before, KLIST_FFL_ARGS)
{
	...
	if (!before) {
		quithere(ERR_CREATE_EXIT, "%s() (%s) can't before a null item" KLIST_FFL,
...
void _k_insert_after(K_LIST *list, K_ITEM *item, K_ITEM *after, KLIST_FFL_ARGS)
{
	...
	if (!after) {
		quithere(ERR_CREATE_EXIT, "%s() (%s) can't after a null item" KLIST_FFL,
				__func__, list->name, KLIST_FFL_PASS);
	}
...
void _k_list_transfer_to_head(K_LIST *from, K_LIST *to, KLIST_FFL_ARGS)
{
	if (from->name != to->name) {
		quithere(ERR_CREATE_EXIT, "List %s can't %s() to a %s list" KLIST_FFL,
...
K_LIST *_k_free_list(K_LIST *list, KLIST_FFL_ARGS)
{
	...
	if (list->is_store) {
		quithere(ERR_CREATE_EXIT, "List %s can't %s() a store" KLIST_FFL,
				list->name, __func__, KLIST_FFL_PASS);
	}
...
{
	if (!(store->is_store)) {
		quithere(ERR_CREATE_EXIT, "Store %s can't %s() the list" KLIST_FFL,
